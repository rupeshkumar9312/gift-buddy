...
export function TrackOrdersClient() {
  ...
  const lookup = async (orderNumberValue: string, emailValue: string) => {
    setError(null);
    setSubmitting(true);
    setOrder(null);
    ...
    setCancelError(null);
    try {
      const result = await trackOrder(orderNumberValue.trim(), emailValue.trim());
      setOrder(result);
    } catch (err) {
  ...
  // A tracking link (e.g. shared over WhatsApp) can carry ?order=&email= so
  // the customer sees their order immediately instead of retyping both.
  // Deferred a tick only to satisfy the lint rule against synchronous
  // setState in an effect body — `lookup` itself still only ever runs once,
  // for the link that landed on the page.
  useEffect(() => {
    const prefilledOrder = searchParams.get("order");
    const prefilledEmail = searchParams.get("email");
    if (prefilledOrder && prefilledEmail) {
      Promise.resolve().then(() => lookup(prefilledOrder, prefilledEmail));
    }
