...
const EMPTY_FORM: PromoBannerInput = {
  eyebrow: "",
  heading: "",
  subtitle: "",
  ctaLabel: "",
...
export default function PromoBannersPage() {
  ...
  const [editing, setEditing] = useState<AdminPromoBanner | null>(null);
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState<PromoBannerInput>(EMPTY_FORM);
  const [error, setError] = useState<string | null>(null);
  const [submitting, setSubmitting] = useState(false);

  const load = () => {
    if (!accessToken) return;
    getPromoBanners(accessToken)
      .then(setBanners)
      .catch(() => undefined);
  };
  ...
  const openEdit = (banner: AdminPromoBanner) => {
    setEditing(banner);
    setForm({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (editing) {
        await updatePromoBanner(accessToken, editing.id, input);
      } else {
        await createPromoBanner(accessToken, input);
      }
      setShowForm(false);
      load();
    } catch (err) {
  ...
  const handleToggleActive = async (banner: AdminPromoBanner) => {
    if (!accessToken) return;
    try {
      await updatePromoBanner(accessToken, banner.id, { isActive: !banner.isActive });
      load();
    } catch (err) {
  ...
  const handleDelete = async (banner: AdminPromoBanner) => {
    if (!accessToken) return;
    if (!confirm(`Delete banner "${banner.heading}"?`)) return;
    try {
      await deletePromoBanner(accessToken, banner.id);
      load();
    } catch (err) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">
            The pair of promo cards on the homepage. Hidden entirely when there are none active.
          </p>
        </div>
        {canWrite && (
          <button
        ...
        >
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <label className="flex flex-col gap-1.5 text-sm">
              <span className="text-muted">Eyebrow (script text)</span>
              <input
                value={form.eyebrow}
      ...
      <div className="flex flex-col gap-3">
        ...
        {banners?.length === 0 && (
          <p className="rounded-2xl border border-line bg-white px-5 py-8 text-center text-sm text-muted">
            No promo banners yet — the homepage section stays hidden until you add one.
          </p>
        )}
      </div>
    </div>
